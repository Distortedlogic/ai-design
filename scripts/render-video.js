...
(async () => {
  ...
  if (hasReady) {
    // Second line of defense: actively reset animation time to zero — handles cases where
    // the HTML doesn't strictly follow the starter tick template (e.g. lastTick uses
    // performance.now() which causes font loading time to be counted into first-frame dt).
    ...
    if (seekCorrected) {
      // Wait for two rAFs to let seek take effect and render the t=0 frame
